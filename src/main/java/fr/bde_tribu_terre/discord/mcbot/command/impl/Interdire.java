...
public class Interdire extends CommandAction {
    public Interdire(@Nonnull final SlashCommandInteractionEvent event, @Nonnull final Bdd bdd) {
        super(event, bdd);
    }
    ...
    public void action() {
        try {
            ...
            if (equipeAutorise.equals(equipe)) {
                this.sendFailure(
                        "Joueur est membre de l'équipe",
                        autorise.getDiscordTag(this.getEvent().getJDA()) +
                                " est déjà pleinement membre de l'équipe \"" + equipeAutorise.getNom() + "\"."
                );
                return;
            }

            // Est-ce que l'autorisé n'est pas autorisé ?
            if (equipe.getAutorises().getJoueurByUuid(autorise.getUuid()) == null) {
                this.sendFailure(
                        "Joueur non autorisé",
                        autorise.getDiscordTag(this.getEvent().getJDA()) +
                                " n'est pas autorisé à accéder à la région de l'équipe \"" + equipe.getNom() + "\"."
                );
                return;
            }

            // Création de la nouvelle équipe.
            List<Joueur> newAutorises = new ArrayList<Joueur>(equipe.getAutorises().getJoueurs());
            newAutorises.remove(autorise);
            ...
            this.sendSuccess(
                    membreCible.getAsTag() + " n'est plus autorisé à accéder à votre région."
            );
